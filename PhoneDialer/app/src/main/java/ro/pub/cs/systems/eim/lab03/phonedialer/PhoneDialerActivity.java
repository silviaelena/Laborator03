...
import ro.pub.cs.systems.eim.lab03.phonedialer.Constants;
...
//        setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_PORTRAIT);
    ...
    private class BackspaceButtonClickListener implements View.OnClickListener {
        @Override
        public void onClick(View view) {
            String phoneNr = phoneNumberEditText.getText().toString();
            if (phoneNr.length() > 0) {
                phoneNr = phoneNr.substring(0, phoneNr.length() - 1);
                phoneNumberEditText.setText(phoneNr);
            }
        }
    }
